...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    ############## LoRA forward step
    lora_in_left, lora_in_right, lora_out_left, lora_out_right = ma_module.in_proj_left, ma_module.in_proj_right, ma_module.out_proj_left, ma_module.out_proj_right

    residual_acti0 = F.linear(query, in_proj_weight, in_proj_bias)
    lora_acti0 = lora_in_right(query)
    lora_acti0 = lora_in_left(lora_acti0)
    acti = residual_acti0 + lora_acti0
    ##############
    ...
    lora_acti1 = lora_out_right(attn_output)
    lora_acti1 = lora_out_left(lora_acti1)
    attn_output = residual_acti1 + lora_acti1
    #############
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, args, dropout=0., bias=True,
        ...
        self.in_proj_left = LoraLinear(args.k, 3 * self.attn_embed_dim)
        self.in_proj_right = LoraLinear(embed_dim, args.k)
        self.out_proj_left =  LoraLinear(args.k, embed_dim)
        self.out_proj_right = LoraLinear(self.attn_embed_dim, args.k)
        ## 
        if add_bias_kv:
    ...
    def reset_LoRA_parameters(self):
        ...
        nn.init.constant_(self.in_proj_left.weight, 0.)
        nn.init.constant_(self.out_proj_left.weight, 0.)

        self.in_proj_right.weight.data = self.in_proj_right.weight.data.float()
        self.out_proj_right.weight.data = self.out_proj_right.weight.data.float()
