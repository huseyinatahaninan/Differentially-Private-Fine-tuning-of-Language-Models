...
from fairseq.adapter_utils import process_batch_grad
...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        for p in self.model.named_parameters():
            if('left' in p[0] or 'right' in p[0] or 'sentence_classification_head' in p[0]):  # only compute the gradients for lora and classification parameters.
                params.append(p[1])
                print('adding %s to params list'%p[0], ', shape: ', p[1].shape)
                if self.args.sigma > 0:
                    p[1].requires_grad = False ## Do not compute the conventional gradients because we will manually compute the indiviudal gradients. 
                else:
                    p[1].requires_grad = True
            else:
                if(counter > 0): ## we need reuqires_grad = True for at least one layer otherwise pytorch will raise error when calling backward
                    p[1].requires_grad = False # disable gradient computation of the original weights to speed up training
            counter += 1
            ...
            try:
                with maybe_no_sync():
                    ...
                    if(self.args.sigma > 0):
                        ...
                        for i, p in enumerate(self.params): #clipping the per-sample gradients and sum them up.
                            if p.grad == None: 
                                p.grad = process_batch_grad(p.batch_grad, scale=scale)
                            else:
                                p.grad += process_batch_grad(p.batch_grad, scale=scale)
