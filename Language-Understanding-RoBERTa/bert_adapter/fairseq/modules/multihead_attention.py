...
def multi_head_attention_forward(query,                           # type: Tensor
    ...
    proj_left, proj_right =  ma_module.proj_left, ma_module.proj_right
    adapter_attn = attn_output
    adapter_attn = proj_right(adapter_attn)
    adapter_attn = F.gelu(adapter_attn)
    adapter_attn = proj_left(adapter_attn)
    attn_output = attn_output + adapter_attn
    #############
...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, args, dropout=0., bias=True,
        ...
        ## new added
        self.proj_left =  AdapterLinear(args.k, embed_dim)
        self.proj_right = AdapterLinear(embed_dim, args.k)
    ...
    def reset_Adapter_parameters(self):
        ...
        nn.init.constant_(self.proj_left.weight, 0.)

        self.proj_right.weight.data = self.proj_right.weight.data.float()

        nn.init.xavier_normal_(self.proj_right.weight)

        if(self.args.fp16):
            self.proj_right.weight.data = self.proj_right.weight.data.half()
    ...
    #     # this is to update the low-rank parameters L and R at in and out proj 
